...
#include "task_rs485_log.h"// 引入 通讯 业务
//#include "task_XKC_Y20_V.h"// 引入 XKC_Y20_V 液位传感器业务
...
void MX_FREERTOS_Init(void) {
  ...
  BSP_I2C1_MutexInit();  /* I2C1 总线互斥锁 (SHT30 + EEPROM 共用) */
...
void StartTask_RS485(void const * argument)
{
  ...
  /* Infinite loop */
Task_RS485Log_Process(argument); // 调用核心业务
  for(;;) { osDelay(1); }          // 理论保底
...
void StartTask03(void const * argument)
{
  ...
  /* Infinite loop */
Task_Panel_Process(argument); // 负责面板、按键、显示屏、报警系统等
  for(;;) { osDelay(1); }
